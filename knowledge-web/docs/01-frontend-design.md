...
| 正文字体 | 自托管子集化 Noto Serif SC（OFL-1.1），回落到 Songti SC / SimSun 等本机栈——正文是文言文，衬线是语义正确的选择，不是装饰 |
| 题注字体 | 自托管子集化霞鹜文楷（OFL-1.1），回落到本机楷体栈——刻本用宋楷分工区隔正文与题注，用在阅读器标题与段落题头 |
...
│       ├── css/app.css
│       ├── css/fonts.css       # 生成物：子集化 webfont 清单，不进版本库
│       ├── fonts/{song,kai}/   # 生成物：unicode-range 分片
│       └── js/{debug,reader,jobs}.js
├── deploy/build-fonts.py       # 部署期抓取并切分字体；不跑也能起
└── tests/
```
