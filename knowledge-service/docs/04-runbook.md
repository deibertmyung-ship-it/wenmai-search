...
**报告里的“到书里看”无法定位** — Web 端链接会携带报告保存的 `version` 与
`[source_start, source_end)` 坐标，服务端从该历史版本返回窗口并精确高亮。若返回
`passage_location_unavailable`，说明该版本的投影没有完整字符偏移映射；这是可解释的
降级，不要改读当前版本或手工移动偏移。来源已被 ACL 撤销/删除时，报告卡片会隐藏定位
按钮，只保留不可访问提示。
